$(function() {
	...
	initeTestType();
	initeTestnets();
	initeWorkerOrdernets();
	 var optInit = {callback: 
	    	function(page_index,jq){
	        $.ajax({
	        	type:"post",
	    		url: contextPath + "/workorder/workorderlist/template",
	    		data:({sernos : $("#hiddenval").val(),
	    		    ...
	    		    jobtype:jobtype,
	    		    testnet:testnet,
	    		    workerOrderNet:workerOrderNet,
	    		    workerOrderNetName:workerOrderNetName,
	    			areaids:$("#areaids").val(),
	    			verify:verify,
	    			s_id:s_id}),
	    		success:function(data){
	    			...
	    			$(".compare_test").click(function(){
	    			   ...
	    			   var src=contextPath+"/report/findFlowid?serialno="+snoid+"&s_id="+s_id+"&areaid="+areaid;
	    				$("#win").dialog({
	    					href:src,
...
//	    					}
	    					$('#win').dialog('open');
	    					window.location.href=contextPath+"/report/reportCompare?flowid="+s+"&flowname="+s_na+"&serialno="+snoid+"&s_id="+s_id+"&areaid="+areaid;
	    				});
	    			});
	    		}
	        });
...
function search(){
	...
	workerOrderNet = $("#workerOrderNet").val();
	workerOrderNetName = $("#workerOrderNetName").val();
...
function exportExcel(){
	$(".right1").show();
	$.ajax({
		url:contextPath + '/workorder/workorderlist/export',
		type:"post",
		data:({sernos : $("#hiddenval").val(),
			isDeal:isDeal,
			startTime:startTime,
		...
		success:function(data){
			$(".right1").hide();
			if(data!="-1" && data!=null){
				window.location.href=contextPath+"/workorder/downLoadExcel?name="+data;
			}else{
