$(function(){
	...
	$('.add').click(function(){
		$("#dlg").dialog({
			href:contextPath + '/ftp4g/addftp',
			height: 550,width: 435,title: "新增FTP配置",
	...
	var optInit = {callback:function(page_index,jq){
			$.ajax({
				type:"post",
	    		url: contextPath + "/ftp4g/updateFtp/template",
	    		...
	    		success: function(data){
					...
					//修改页面打开
				  	$('.edit').click(function(){
				  		var id = $(this).attr("id");
						$("#dlg").dialog({
							href:contextPath + '/ftp4g/editftp?id='+id,
							height: 550,width: 435,title: "修改FTP配置",
							modal: true,closed:false
						});
						$('#dlg').dialog('open');
						$.parser.parse('#dlg');
						...
						if(sid==0){
							$.messager.confirm("提示","确认启用此项配置！",function(r){	
								if(r){
							  		$.ajax({
							  			type:'post',
									  	url: contextPath + "/ftp4g/staUse",
									  	data: ({id:id}),
									  	success:function(data) {
								        	if(data.msg==1){
								        		$.messager.alert("提示","启用成功！","success",function(){
													location.href= contextPath + "/ftp4g/updateFtp";
												});
								        	}else if(data.msg==0){
