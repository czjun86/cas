...
function initializePage() {
	...
	$(".netclass").click(
			function() {
				...
				var ss = contextPath + "/report/reportlist?serialno="
...
function ajaxChangeSel() {
	...
	$.ajax({
		type : "post",
		url : contextPath + "/report/changelist",
		data : ({
			serialno : serid,
			testType : testtype,
			areaid:$("#areaid").val(),
		...
		success : function(data) {
			...
			if (fl.length > 0) {
				...
				for ( var i = 0; i < fl.length; i++) {
					...
					var net_system=fl[i].netsystem+0;
						var netstr = "";
						if(net_system==1){netstr="_GSM";}
						if(net_system==2){netstr="_WCDMA_锁频";}
						if(net_system==3){netstr="_WCDMA_自由模式";}
						if(net_system==4){netstr="_LTE_锁频";}
						if(net_system==5){netstr="_LTE_自由模式";}	
						temp += netstr;
...
function getData(flowid) {
	...
	$
			.ajax({
				type : "post",
				url : contextPath + "/report/reportlist",
				data : ({
					flowid : flowid,
					areaid:$("#areaid").val()
				}),
				success : function(data) {
					...
					for ( var i = 0; i < flist.length; i++) {
						...
						if (ylist.length > 0) {
							...
							if (flist[i].kpiId <= 8
									|| (flist[i].kpiId >= 20 && flist[i].kpiId !=22&& flist[i].kpiId < 27)) {
									...
									$("#histogram2")
											.append(
													"<div class='histogram_flash' id='"
															...
															+ wid_len
															+ "px;margin:0 auto;position: relative;'></div>");
									var kk_name=flist[i].kpiname+(flist[i].netType=="2"?"(3G)":"(4G)");
									creatFlash(indoor_x, xlist, ylist, idname,
											null, colorlist, kk_name,
					...
					if (sernolist && sernolist.length > 0) {
						...
						sernolist[0].netsystem = sernolist[0].netsystem == 3 ? 'WCDMA自由模式'
								: sernolist[0].netsystem;
						sernolist[0].netsystem = sernolist[0].netsystem == 4 ? 'LTE锁频'
								: sernolist[0].netsystem;
						sernolist[0].netsystem = sernolist[0].netsystem == 5 ? 'LTE自由模式'
								: sernolist[0].netsystem;
...
//						增加noService行用于添加3G,2G,以及无服务占比
						...
						var noservice_eval="";
							...
							if(compEval[2]!=null){
								wcdma_eval="<div><div style='float:left; display:block;'>3G：</div><span style='background:"+compEval[2]+";float:left;width:22px; height:22px;display:block; text-align:center; line-height:22px; color:#FFF; margin-right:25px;margin-top:1px;'>"+compEval[3]+"</span></div>";
							}
							if(compEval[4]!=null){
								lte_eval="<div><div style='float:left; display:block;'>4G：</div><span style='background:"+compEval[4]+";float:left;width:22px; height:22px;display:block; text-align:center; line-height:22px; color:#FFF; margin-right:25px;margin-top:1px;'>"+compEval[5]+"</span></div>";
							}
							if(compEval[0]==null&&compEval[2]==null&&compEval[4]==null){
								noservice_eval="无";
							}
						htl.push("<tr>");
						htl.push("<td>4G占比：</td><td id='lteScale' name='exc_lteRatio'></td>");
						htl.push("<td>脱网率：</td><td id='noServiceScale' name='exc_noService'></td>");
						htl.push("</tr>");
						htl.push("<tr><td>本次综合评价：</td><td name='exc_evaluate'  td colspan='3' style='border-top:1px'>"+lte_eval+wcdma_eval+gsm_eval+noservice_eval+"</td></tr>");
						htl.push("<tr>");
...
function showIndoorMap(lat,lng,type,address){
	...
	if(type==1){
		...
		$("#indoormap").dialog({
			href:contextPath + '/report/indoorMap?lat='+lat+'&lng='+lng+'&flowid='+flowid,
		...
		$("#indoormap").dialog({
			href:contextPath + '/report/showAddress',
		...
		$(".saveedit").unbind("click").click(function(){
			...
			$('#dataForm').ajaxForm({
			  	url: contextPath + '/report/updateAddress?flowid='+flowid+"&areaid="+$("#areaid").val(),
			    success: function(data) {
			      if(data.msg == 1){
...
function creatFlash(indoor_x, xlist, ylist, kpiname, ftp, colorlist, title,
	...
	if (ylist[0].kpi != '20' && ylist[0].kpi != '21' && ylist[0].kpi != '26'&& !ftp) {
...
$(function(){
	...
	$(".saveVerify").live('click',function(){
		...
		$.ajax({
			type : "post",
			url : contextPath + "/report/updateVerify",
			data : ({
				id : id,
				val : val
			}),
