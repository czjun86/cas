...
var markerArr_2g=[];
var markerMap_fliwids_4g = new Map();//KEY:流水号;value:marker数组
var markerArr_4g=[];
...
var markerArr_3g_ex=[];
var markerMap_fliwids_4g_ex = new Map();//KEY:流水号;value:偏移前地址数组
var markerArr_4g_ex=[];
...
function getData(flowid,flowname)
{
	$("#tuli_id").click( function () { $("#tuli_ul").toggle(); });
	 ...
	 //请求数据
	   	$.ajax({
					type:"post",
					url:contextPath+"/report/reportCompare",
					...
					success:function(data){
						    ...
						    var ltelist=data.ltelist;
						    var center=data.center;
						    queryPoint(gsmlist,wcdmalist,ltelist,center,flist,flowid,flowname);
						    ...
						    if(ylist.length>0){
								  ...
								  if(flist[i].kpiId<=8||(flist[i].kpiId >= 20 && flist[i].kpiId !=22&& flist[i].kpiId < 27)){
									    ...
									    if(flist[i].kpiId!=4&&flist[i].kpiId!=5)
										  {
										     $("#histogram1").append("<div class='histogram_flash' id='"+idname+"'  style='background:url(../../img/bb.png) no-repeat;width:"+wid_len+"px;margin:0 auto;position: relative;'></div>");
										      ...
										      creatFlash(indoor_x,xlist,ylist,idname,null,colorlist, kk_name);
										      //FTP曲线图
										      if(ftp&&ftp.length>0){
