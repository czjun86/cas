...
function initMap(wcdmalist, gsmlist,ltelist, center, psc_xlist, bcch_xlist,pci_xlist) {
	...
	// 默认显示右边数据为第一个点数据
	var time_3g, time_2g,time_4g, id_3g,id_4g, id_2g, lat_3g,lat_4g, lng_3g, lng_4g,lat_2g, lng_2g;
	var time_3g_last,time_4g_last,time_2g_last,id_3g_last,id_4g_last,id_2g_last;
	if (gsmlist) {
	...
	if (ltelist) {
		for ( var i = 0; i < ltelist.length; i++) {
			if (i == 0) {
				time_4g = ltelist[i].eptime;
				id_4g = ltelist[i].id;
				lat_4g = ltelist[i].lat_modi;
				lng_4g = ltelist[i].lng_modi;
			}
			break;
		}
		if(ltelist.length>0){
	...
	google.maps.event.addListener(map, 'maptypeid_changed', function() {
		...
		if (map.getMapTypeId() == google.maps.MapTypeId.ROADMAP) {// 判断是否普通地图
			...
			for ( var i = 0; i < latAndlngMap_4g.size(); i++) {
				var marker = latAndlngMap_4g.elements[i].key;
				marker.setPosition(latAndlngMap_4g.get(marker));
			}
		} else {
			// 卫星地图
			...
			for ( var i = 0; i < exlatAndlngMap_4g.size(); i++) {
				var marker = exlatAndlngMap_4g.elements[i].key;
				marker.setPosition(exlatAndlngMap_4g.get(marker));
			}
		}
	});
}
...
function chooseKpi(wcdmalist, gsmlist, ltelist,kpi, type, psc_xlist, bcch_xlist,pci_xlist) {
	...
	for ( var t = 0; t < str.length; t++) {
		...
		// ftp
		case 4:
			var list_dang=ltelist.length>0?ltelist:wcdmalist;
			var tt_p=ltelist.length>0?3:2;
			for ( var i = 0; i < list_dang.length; i++) {
				addMarker(list_dang[i].ftpSpeed, list_dang[i].id, tt_p,
						new google.maps.LatLng(list_dang[i].lat_modi + t
								* (0.01 / zoom), list_dang[i].lng_modi - t
								* (0.01 / zoom)), i, "FTP",
						new google.maps.LatLng(list_dang[i].lat + t
								* (0.01 / zoom), list_dang[i].lng - t
								* (0.01 / zoom)), list_dang[i].ftpSpeed_,
								list_dang[i].realnet_type);
			}
			break;
		// rxlev
		case 6:
		...
		case 23:
			for ( var i = 0; i < ltelist.length; i++) {
				addMarker(ltelist[i].rsrp, ltelist[i].id, 3,
						new google.maps.LatLng(ltelist[i].lat_modi + t
								* (0.01 / zoom), ltelist[i].lng_modi - t
								* (0.01 / zoom)), i, "RSRP",
						new google.maps.LatLng(ltelist[i].lat + t
								* (0.01 / zoom), ltelist[i].lng - t
		...
		case 25:
			for ( var i = 0; i < ltelist.length; i++) {
				addMarker(ltelist[i].snr, ltelist[i].id, 1,
						new google.maps.LatLng(ltelist[i].lat_modi + t
								* (0.01 / zoom), ltelist[i].lng_modi - t
								* (0.01 / zoom)), i, "SINR",
						new google.maps.LatLng(ltelist[i].lat + t
		...
		case 26:
			...
			for ( var i = 0; i < ltelist.length; i++) {
				...
				if (ij < 20) {
					addMarker(dc[ij], ltelist[i].id, 1, new google.maps.LatLng(
							ltelist[i].lat_modi + t * (0.01 / zoom),
							ltelist[i].lng_modi - t * (0.01 / zoom)), i,
							"PCI", new google.maps.LatLng(ltelist[i].lat + t
									* (0.01 / zoom), ltelist[i].lng - t
...
function addMarker(state, id, type, location, index, kpi, exlocation, intevl,
	...
	if (marker) {
		...
		if (type == 1) {
			...
			latAndlngMap_4g.put(marker, location);
		}
