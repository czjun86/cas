...
function drawStation(cells, map) {
	...
	if (cells.length >= 1) {
		...
		for ( var j = 0; j < cells.length; j++) {
			var num = cells[j].celllat;
			cells[j].celllat = cells[j].celllng;
			cells[j].celllng = num;
		...
		for (ss in xxbhMap) {
			...
			if (html != '') {
				...
				var latLng = new BMap.Point(st[0].celllat, st[0].celllng);
				st[0].isclick=st[0].isclick==undefined?0:st[0].isclick;
				st[0].isdelet=st[0].isdelet==undefined?0:st[0].isdelet;
               //谷哥经纬度转换为百度经纬度
				/* BMap.Convertor.translate(latLng,2,function(p)
						{
					 s = new stationLayer(st, html, map, p,st[0].isclick,st[0].baseName.substring(start_dex,st[0].baseName.length),st[0].isdelet);
						map.addOverlay(s); 
...
stationLayer.prototype.initialize = function() {
	...
	$(div).html(this.text_);
	this.div_ = div;
	this.map_.getPanes().markerPane.appendChild(div);   
	rotateStation(this.data_, this.map_,this.isclick_);
};
var overlayProjection;
stationLayer.prototype.draw = function() {
	var latLng = this.map_.pointToOverlayPixel(this.latLng_);
	var div = this.div_;
	var ww = 6;
	var hh = 12;
	var size = new BMap.Size(ww, hh);
...
stationLayer.prototype.toggle = function() {
	 if (this._div){    
		   if (this.div_.style.display == ""){    
			...
			for(var i=0;i<len;i++){
				...
				$("#img_" + _data[i].lac + "_" + _data[i].cellId).unbind().rotate({
					bind : {
						click : function() {
							var id = $(this).attr('id');
							t=setTimeout(function(){showcells(id,_map,$(this))},100);
...
//								}
									setTimeout(function(){cleanCellImg()},300);
							}
						},
						/*click:function(){
							//if(_isclick!=1){
 ...
 * @param lac
 */
var opts = {enableMessage:false,enableCloseOnClick:false};
function showcells(id,map,obj){
	 $.post(contextPath + "/gisgrad/showContent",
				...
				function(data){
					...
					var html="";
					if(cell.modetype==0){
						 html = new EJS({url: contextPath+'/js/reportIndependent/cell_info.ejs'}).render(ccg);
					}else if(cell.modetype==1){
						html = new EJS({url: contextPath+'/js/reportIndependent/cell_info_gsm.ejs'}).render(ccg);
					}
					var jinwei=new BMap.Point(cell.celllng,cell.celllat);
					var infoWindow = new BMap.InfoWindow(html, {enableMessage:false,enableCloseOnClick:false});  // 创建信息窗口对象   
						map.openInfoWindow(infoWindow, jinwei);      // 打开信息窗口
...
function getlanlngBypx(cc,obj){
	...
	var latLng= new BMap.Point(cc.celllat,cc.celllng);
	...
	//扇叶边缘经纬度
	var ln=overlayProjection.fromDivPixelToLatLng(new BMap.Point(pp.x + xMargin,pp.y + yMargin));
	return ln;
}
