...
{ 
	...
	var s_id=$("#s_id").val();
	var src=contextPath+"/reportIndependent/findFlowid?serialno="+$("#ss_id").html()+"&flowid="+dd+"&areaid="+areaid+"&s_id="+s_id;
...
//			str += arr[i]+ ',';
//		}
		window.location.href=contextPath+"/reportIndependent/reportCompare?flowid="+s+"&flowname="+s_na+"&serialno="+$("#ss_id").html()+"&areaid="+areaid+"&s_id="+s_id;
	});
}
...
function choose_demo(flowid,flist,sel1,sel2,sel3,sel1_n,sel2_n,sel3_n,div_name,wcdmalist,gsmlist,ltelist){
		   ...
		   for(var i=0;i<flist.length;i++){
			   if(flist[i].kpiId==parseInt(sel2)||flist[i].kpiId==parseInt(sel1)||(flist[i].kpiId==5&&parseInt(sel2)==4))
				 {
				   var demo_div=[]; 
				    ...
				    var x_list=[];
					   ...
					   for (var j=0;j<x_list.length;j++){
						   if(x_list[j]!=""&&yy.length>0){
							   if(flist[i].kpiId!='20'&&flist[i].kpiId!='21'){
		  ...
		  var c= demo_div_2g.concat(demo_div_3g);
