...
function queryPoint(gsmlist,wcdmalist,center,flist,flowids,flowname){
    ...
    for(ss in xxMap){
       ...
       if(st[0].inside==0){
   	      //生成地图    
   	     var  map = new google.maps.Map(document.getElementById("map_"+ss), myOptions_dt);
   	     var ija=$.inArray(ss,cen_arr_f);
   	     if(ija>=0){
   	    	var Item_1 = new google.maps.LatLng(center[ija].max_lat ,center[ija].max_lng);
   			var myPlace = new google.maps.LatLng(center[ija].min_lat, center[ija].min_lng);
     	...
     	map.setCenter(new google.maps.LatLng(29.559247,106.54241));
     	map.setZoom(15);
     }
   	     map_arr.push(map);
   	  //下拉选择事件
	   	   ...
	   	   if(d_1.length>0){
	   		$("#g2_"+s_fid).combobox({
	   	     ...
	   	     if(d.length>0){
				 ...
				 //卫星地图切换事件
			 if(st[0].inside==0){
			   	    google.maps.event.addListener(p, 'maptypeid_changed', function(){
			   	    		 var arr2=markerMap_fliwids_2g_ex.get($(p.getDiv()).attr("id").split("_")[1]);
			   	    		 var arrm2=markerMap_fliwids_2g.get($(p.getDiv()).attr("id").split("_")[1]);
			   	    		 for(i in arrm2){
			   	    			var lo=arrm2[i].getPosition();
			   	    			arrm2[i].setPosition(arr2[i]);
			   	    			arr2[i]=lo;
			   	    		 }
			   	    		 markerMap_fliwids_2g_ex.remove(s_fid);
			   	    		 markerMap_fliwids_2g_ex.put(s_fid,arr2);
			   	    		 var arr3=markerMap_fliwids_3g_ex.get($(p.getDiv()).attr("id").split("_")[1]);
			   	    		 var arrm3=markerMap_fliwids_3g.get($(p.getDiv()).attr("id").split("_")[1]);
			   	    		 for(i in arrm3){
			   	    			var lo=arrm3[i].getPosition();
			   	    			arrm3[i].setPosition(arr3[i]);
			   	    			arr3[i]=lo;
			   	    		 }
			   	    		 markerMap_fliwids_3g_ex.remove(s_fid);
			   	    		 markerMap_fliwids_3g_ex.put(s_fid,arr3);
			   	    });
		   	   }
...
function outclick(ss,map_arr,wcdmalist,gsmlist,yy_p,yy_b){
	var map=null;
	for(t in map_arr){
		if($(map_arr[t].getDiv()).attr("id").split("_")[1]==ss){
			map=map_arr[t];
		}
	}
	$('.wrapper,#dialData').hide();
	...
	var gs_len=0;
	 ...
	 for(var i=0;i<wd_len;i++){
				if(wcdmalist[i].rscp>0&&$.inArray('RSCP',str)>=0){
					addMarker(map,wcdmalist[i].rscp,wcdmalist[i].id,2,new google.maps.LatLng(wcdmalist[i].lat_modi+$.inArray('RSCP',str)/(map.getZoom()*100),wcdmalist[i].lng_modi-$.inArray('RSCP',str)/(map.getZoom()*100)),i,"RSCP",
							new google.maps.LatLng(wcdmalist[i].lat+$.inArray('RSCP',str)/(map.getZoom()*100),wcdmalist[i].lng-$.inArray('RSCP',str)/(map.getZoom()*100)),wcdmalist[i].rscp_,wcdmalist[i].realnet_type);
				}
				if(wcdmalist[i].ecno>0&&$.inArray('EcNo',str)>=0){
					addMarker(map,wcdmalist[i].ecno,wcdmalist[i].id,2,new google.maps.LatLng(wcdmalist[i].lat_modi+$.inArray('EcNo',str)/(map.getZoom()*100),wcdmalist[i].lng_modi-$.inArray('EcNo',str)/(map.getZoom()*100)),i,"EC/No",
							new google.maps.LatLng(wcdmalist[i].lat+$.inArray('EcNo',str)/(map.getZoom()*100),wcdmalist[i].lng-$.inArray('EcNo',str)/(map.getZoom()*100)),wcdmalist[i].ecno_,wcdmalist[i].realnet_type);
				}
				if(wcdmalist[i].txpower>0&&$.inArray('TXPOWER',str)>=0){
					addMarker(map,wcdmalist[i].txpower,wcdmalist[i].id,2,new google.maps.LatLng(wcdmalist[i].lat_modi+$.inArray('TXPOWER',str)/(map.getZoom()*100),wcdmalist[i].lng_modi-$.inArray('TXPOWER',str)/(map.getZoom()*100)),i,"TxPower",
							new google.maps.LatLng(wcdmalist[i].lat+$.inArray('TXPOWER',str)/(map.getZoom()*100),wcdmalist[i].lng-$.inArray('TXPOWER',str)/(map.getZoom()*100)),wcdmalist[i].txpower_,wcdmalist[i].realnet_type);
					}
				if(wcdmalist[i].ftpSpeed>0&&$.inArray('FTP',str)>=0){
					addMarker(map,wcdmalist[i].ftpSpeed,wcdmalist[i].id,2,new google.maps.LatLng(wcdmalist[i].lat_modi+$.inArray('FTP',str)/(map.getZoom()*100),wcdmalist[i].lng_modi-$.inArray('FTP',str)/(map.getZoom()*100)),i,"FTP",
							new google.maps.LatLng(wcdmalist[i].lat+$.inArray('FTP',str)/(map.getZoom()*100),wcdmalist[i].lng-$.inArray('FTP',str)/(map.getZoom()*100)),wcdmalist[i].ftpSpeed_,wcdmalist[i].realnet_type);
					}
				if(wcdmalist[i].psc&&$.inArray('PSC',str)>=0)
					...
					//PSC
						 var ij=$.inArray(wcdmalist[i].psc.toString(),yy_p);
						   if(ij<20){
							   addMarker(map,dc[ij],wcdmalist[i].id,2,new google.maps.LatLng(wcdmalist[i].lat_modi+$.inArray('PSC',str)/(map.getZoom()*100),wcdmalist[i].lng_modi-$.inArray('PSC',str)/(map.getZoom()*100)),i,"PSC",
										new google.maps.LatLng(wcdmalist[i].lat+$.inArray('PSC',str)/(map.getZoom()*100),wcdmalist[i].lng-$.inArray('PSC',str)/(map.getZoom()*100)),wcdmalist[i].psc,wcdmalist[i].realnet_type);
					       }
					}
		    }
			...
			for(var i=0;i<gs_len;i++){
				...
				addMarker(map,gsmlist[i].rxlev,gsmlist[i].id,1,new google.maps.LatLng(gsmlist[i].lat_modi+$.inArray('RXLEV',str)/(map.getZoom()*100),gsmlist[i].lng_modi-$.inArray('RXLEV',str)/(map.getZoom()*100)),i,"Rxlev",
						new google.maps.LatLng(gsmlist[i].lat+$.inArray('RXLEV',str)/(map.getZoom()*100),gsmlist[i].lng-$.inArray('RXLEV',str)/(map.getZoom()*100)),gsmlist[i].rxlev_,gsmlist[i].realnet_type);
				}
				if(gsmlist[i].rxqual>0&&$.inArray('RXQUAL',str)>=0){
					addMarker(map,gsmlist[i].rxqual,gsmlist[i].id,1,new google.maps.LatLng(gsmlist[i].lat_modi+$.inArray('RXQUAL',str)/(map.getZoom()*100),gsmlist[i].lng_modi-$.inArray('RXQUAL',str)/(map.getZoom()*100)),i,"Rxqual",
							new google.maps.LatLng(gsmlist[i].lat+$.inArray('RXQUAL',str)/(map.getZoom()*100),gsmlist[i].lng-$.inArray('RXQUAL',str)/(map.getZoom()*100)),gsmlist[i].rxqual_,gsmlist[i].realnet_type);
					}
				if(gsmlist[i].ci>0&&$.inArray('C/I',str)>=0){
					addMarker(map,gsmlist[i].ci,gsmlist[i].id,1,new google.maps.LatLng(gsmlist[i].lat_modi+$.inArray('C/I',str)/(map.getZoom()*100),gsmlist[i].lng_modi-$.inArray('C/I',str)/(map.getZoom()*100)),i,"C/I",
							new google.maps.LatLng(gsmlist[i].lat+$.inArray('C/I',str)/(map.getZoom()*100),gsmlist[i].lng-$.inArray('C/I',str)/(map.getZoom()*100)),gsmlist[i].ci_,gsmlist[i].realnet_type);
					}
				if(gsmlist[i].bcch&&$.inArray('BCCH',str)>=0){
					
					 var ij=$.inArray(gsmlist[i].bcch.toString(),yy_b);
					   if(ij<20){
						   addMarker(map,dc[ij],gsmlist[i].id,1,new google.maps.LatLng(gsmlist[i].lat_modi+$.inArray('BCCH',str)/(map.getZoom()*100),gsmlist[i].lng_modi-$.inArray('BCCH',str)/(map.getZoom()*100)),i,"BCCH",
									new google.maps.LatLng(gsmlist[i].lat+$.inArray('BCCH',str)/(map.getZoom()*100),gsmlist[i].lng-$.inArray('BCCH',str)/(map.getZoom()*100)),gsmlist[i].bcch,gsmlist[i].realnet_type);
				       }
				}
...
function addMarker(map,state,id,type,location,index,kpi,exlocation,intevl,realnetType){
	intevl=":"+intevl;
	var scale=1;
