$(function() {
	...
	initeTestnets();
	 var optInit = {callback: 
	        ...
	        $.ajax({
	        	type:"post",
	    		url: contextPath + "/task/tasklist/template",
	    		...
	    		success:function(data){
	    			...
	    			$(".compare_test").click(function(){
	    			   ...
	    			   var src=contextPath+"/reportTask/findFlowid?serialno="+snoid+"&s_id="+s_id+"&areaid="+areaid;
	    				$("#win").dialog({
	    					href:src,
...
//	    					}
	    					$('#win').dialog('open');
	    					window.location.href=contextPath+"/reportTask/reportCompare?flowid="+s+"&flowname="+s_na+"&serialno="+snoid+"&s_id="+s_id+"&areaid="+areaid;
	    				});
	    			});
	    		}
	        });
	...
	$("#areas").combobox({
		onShowPanel:function(){
			...
			var src = contextPath + '/task/arealist?areaids='+$("#areaids").val()+"&type=0";
			$("#areadlg").dialog({
				href:src,
...
function search(){
	...
	breakflag=$("#breakflag").combobox("getValue");
