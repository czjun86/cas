	...
	if (cells.length >= 1) {
		...
		for (ss in xxbhMap) {
			...
			if (html != '') {
				...
				var latLng = new BMap.Point(st[0].celllng,st[0].celllat);
...
function showcells(id,map,obj){
	 $.post(contextPath + "/gisgrad/showContent",
				...
				function(data){
					...
					var html="";
					if(ccg.modetype==0){
						 html = new EJS({url: contextPath+'/js/gisgrad/cell_info.ejs'}).render(ccg);
					}else if(ccg.modetype==1){
						html = new EJS({url: contextPath+'/js/gisgrad/cell_info_gsm.ejs'}).render(ccg);
					}
...
function getlanlngBypx(cc,obj){
	...
	var latLng= new BMap.Point(cc.celllng,cc.celllat);
	var pp =_map.pointToOverlayPixel(latLng);
	var radian = (cc.azimuth-90) * Math.PI / 180; 
	var xMargin = Math.cos(radian) * dis;
	var yMargin = Math.sin(radian) * dis;
	//扇叶边缘经纬度
	var ln=_map.overlayPixelToPoint( new BMap.Pixel(pp.x + xMargin,pp.y + yMargin));
	return ln;
}
...
function removeStationMap(){
	for ( var i = 0; i < stationMap.length; i++) {
		_map.removeOverlay(stationMap[i]);
	}
	stationMap = [];
	if(polyline){
		_map.removeOverlay(polyline);
