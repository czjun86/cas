	...
	$.post(contextPath + "/gisgrad/gisgrad",
			...
			function(data){
	           ...
	           for(var i=0;i<list_marker.length;i++){
	      	  ...
	      	  markerCluster = new BMapLib.MarkerClusterer(map,{markers:markerlist,maxZoom:DEFAULTMAPZOOM-1});
...
function initeMap(){
	...
	map.centerAndZoom(new BMap.Point(106.5,29.5) ,14);
	...
	map.addEventListener('dragend', function(re) {
		if(map.getZoom()>=DEFAULTMAPZOOM){
			   changeBound(map);
			}else{
				removeStationMap();
    ...
    geocoder = new BMap.Geocoder(); //申明地址解析对象
		...
		if(map.getZoom()>=DEFAULTMAPZOOM && stationInitData && stationInitData.length>0){
			...
			geocoder.getLocation(point,function(rs){
					if(rs){
						...
						showRightMenu(event.pixel.x, event.pixel.y,point,address);								
					}
			});
	...
	if(_map && _map.positionList){
		for(var i=0,len=_map.positionList.length; i<len; i++){
			...
			if(areaid == item.areaid){
				//清空小区信息
				removeStationMap();
				_map.setCenter(new BMap.Point(item.longitude,item.latitude));
	...
	for(var i = 0; i < stationMap.length; i++){
		...
		if(lat <= latSW || lng <= lngSW || lat >= latNE || lng >= lngNE){
			if(stationMap[i].isdelet_<1){
				//删除小区信息
				map.removeOverlay(stationMap[i]);
...
function addMarker(li,map,colorlist,nettype,index){
	 var position = new BMap.Point(li.lng_m,li.lat_m);
	 var n="";
	 if(li.net==1){n='g';}
	 if(li.net==2){n='w';}
	 if(li.net==3){n='w';}
	...
	var img =new BMap.Icon(icon,new BMap.Size(14,14), {imageSize:new BMap.Size(14,14)});
	var marker=new BMap.Marker(position,{icon:img});
...
function polyine_cell(li,map){
	 $.post(contextPath + "/gisgrad/giscell",
				...
				function(data){
					...
					setTimeout(function(){
						//连线
						markerLatLng_bit=new BMap.Point(li.lng_m,li.lat_m);
						polyline.setPath(countLatLng(cells,markerLatLng_bit));
						map.addOverlay(polyline);
						},200);
				});
...
function setMapCenter(map,center){
	if (center) {
		var Item_1 = new BMap.Point(center.max_lng,center.max_lat);
		var myPlace = new BMap.Point(center.min_lng,center.min_lat);
		var pps=[];
		pps.push(Item_1);pps.push(myPlace);
		map.setViewport(pps);
	}
}
$(function(){
