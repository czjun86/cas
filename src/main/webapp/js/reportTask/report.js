...
function initializePage() {
	...
	$(".netclass").click(
			function() {
				...
				var ss = contextPath + "/reportTask/reportlist?serialno="
...
function ajaxChangeSel() {
	...
	$.ajax({
		type : "post",
		url : contextPath + "/reportTask/changelist",
		data : ({
			serialno : serid,
			testType : testtype,
			areaid:$("#areaid").val(),
...
function getData(flowid) {
	...
	$
			.ajax({
				type : "post",
				url : contextPath + "/reportTask/reportlist",
				data : ({
					flowid : flowid,
					areaid:$("#areaid").val()
				}),
...
function showIndoorMap(lat,lng,type,address){
	...
	if(type==1){
		...
		$("#indoormap").dialog({
			href:contextPath + '/reportTask/indoorMap?lat='+lat+'&lng='+lng+'&flowid='+flowid,
		...
		$("#indoormap").dialog({
			href:contextPath + '/reportTask/showAddress',
		...
		$(".saveedit").unbind("click").click(function(){
			...
			$('#dataForm').ajaxForm({
			  	url: contextPath + '/reportTask/updateAddress?flowid='+flowid+"&areaid="+$("#areaid").val(),
			    success: function(data) {
			      if(data.msg == 1){
...
$(function(){
	...
	$(".saveVerify").live('click',function(){
		...
		$.ajax({
			type : "post",
			url : contextPath + "/reportTask/updateVerify",
			data : ({
				id : id,
				val : val
			}),
