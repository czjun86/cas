   ...
   <form action="${application.getContextPath()}/workorder/workorderlist" id="searchForm" method="post">
      ...
      <span class="wlfspan">开始日期：</span>
	    ...
	    <div class="search_input" style="width:80px;height:28px;">
		      <select id="datatype" class="easyui-combobox" name="datatype" style="width:75px;height:26px;" data-options="editable:false,panelHeight: 'auto'">  
		        <option value="1" <#if datatype?? && datatype == '1'>selected="selected"</#if>>接单时间</option>
	    ...
	    <div class="search_input" style="width:98px;height:28px;">
		      <select id="nettype" class="easyui-combobox" name="nettype" style="width:65px;height:26px;" data-options="editable:false,panelHeight: 'auto'">  
		        ...
		        <option value="4" <#if nettype?? && nettype == '4'>selected="selected"</#if>>LTE</option>
		     </select> 
	    </div>
	    ...
	    <div class="" style="float:left;margin-right:16px;">
         ...
         <span class="wlfspan">投诉网络：</span>
	    <div class="search_input" style="width:98px;height:28px;">
		      <select id="workerOrderNets" class="easyui-combobox" name="workerOrderNets" style="width:65px;height:26px;" data-options="editable:false,panelHeight: 'auto'">  
		        <option value="-1"><#if workerOrderNetName?? && workerOrderNetName !=''>${workerOrderNetName!}<#else>全网络</#if></option>	
		     </select> 
	          <input type="hidden" id="workerOrderNetName" name="workerOrderNetName" value="${workerOrderNetName!'全网络'}"/>
	          <input type="hidden" name="workerOrderNet" value="${workerOrderNet!'-1'}" id="workerOrderNet"/>
	    </div>
	    ...
	    <samp onclick="search()" style="float:right;margin-right:5px;margin-top:-40px;">查询</samp>
	    <samp onclick="exportExcel()" style="float:right;float:right;margin-right:5px;margin-top:-0px;">导出</samp>
      </div>
      </div>
     </form> 
      <div class="clear"></div>
      <div class="all_list">
        ...
        <ul class="all_list_ts">
          <li style="width:16%;" class="all_list_tsleft"><span>投诉工单号
            </span></li>
          <li style="width:8%">区域</li> 
          <li style="width:8%">网络类型</li>
          <li style="width:8%">重要性分类</li>
          <li style="width:15%">系统接单时间</li>
          <li style="width:6%">状态</li>
          <li style="width:15%">测试时间</li>
...
<div id="workerOrderNetdlg" class="easyui-dialog" title="选择投诉网络" style="padding:10px"
		data-options="
			buttons: '#select-workerOrderNetdlg',
			modal:true ,
			closed:true
		">
</div>
<div id="select-workerOrderNetdlg" style="display:none">
	<a href="javascript:void(0)" class="easyui-linkbutton sel_workerOrderNet">确定</a>
	<a href="javascript:void(0)" class="easyui-linkbutton" onclick="javascript:$('#workerOrderNetdlg').dialog('close')">取消</a>
</div>
</@body>
<@script>
	...
	<script src="${application.getContextPath()}/js/workorder/workorder.js" type="text/javascript"></script>
	<script src="${application.getContextPath()}/js/workorder/gradquery.js" type="text/javascript"></script>
	<script>
		...
		var workerOrderNet = <#if workerOrderNet??>'${workerOrderNet!}'<#else>'-1'</#if>;
		var workerOrderNetName = <#if workerOrderNetName??>'${workerOrderNetName!}'<#else>'全网络'</#if>;
		$(function(){
