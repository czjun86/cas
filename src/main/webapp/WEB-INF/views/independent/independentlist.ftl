...
<@body>
  ...
  <div class="work_list" style="width:98%;margin-left:1%">
   <form action="${application.getContextPath()}/independent/independentlist" id="searchForm" method="post">
      <!-----搜索框 开始 ------->
      <div class="wls">
      <div class="wlf">
	    ...
	    <span class="wlfspan" style="margin-left:3px;">测试网络：</span>
	    ...
	    <div class="search_input" style="width:80px;height:28px;">
		      <select id="breakFlag" class="easyui-combobox" name="breakFlag" style="width:65px;height:26px;" data-options="editable:false,panelHeight: 'auto'">  
		        <option value="-1" <#if breakFlag?? && verify == -1>selected="selected"</#if>>全部</option>
		        ...
		        <option value="4" <#if breakFlag?? && breakFlag == 4>selected="selected"</#if>>其它</option>
		     </select> 
	    </div> 
          
	    <span class="wlfspan">测试地址：</span>
        <div class="search_input" style=" margin-right: 10px;width:241px">
          <h6></h6>
          <h5>
          <input  name = "testAddr" value="${testAddr!}" type="text" id="testAddr" style="width:231px"/>
          </h5><h4></h4>
         </div>
      ...
      <div class="all_list">
        ...
        <li style="width:16%;" class="all_list_tsleft"><span>自主工单号</span></li>
          <li style="width:8%">区域</li> 
          <li style="width:8%">测试环节</li>
          <li style="width:12%">创建工单时间</li>
          <li style="width:20%">测试地址</li>
          <li style="width:12%">测试时间</li>
...
<@script>
	...
	<script src="${application.getContextPath()}/js/independent/independent.js" type="text/javascript"></script>
	...
	<script>
		...
		var userid = <#if userid??>${userid!}<#else>-1</#if>;
