...
public class TokenProcessor {
   ...
   public static String FORM_TOKEN_KEY="token_key";
   ...
   public synchronized void mvAddToken(ModelAndView mv,HttpServletRequest request){
	    this.saveToken(request);
	    String token=(String)request.getSession().getAttribute(FORM_TOKEN_KEY);
	    mv.addObject("tokename", FORM_TOKEN_KEY);
	    mv.addObject("tokenkey", token);
   }
   public synchronized void modelAddToken(Model model,HttpServletRequest request){
	    this.saveToken(request);
	    String token=(String)request.getSession().getAttribute(FORM_TOKEN_KEY);
	    model.addAttribute("tokename", FORM_TOKEN_KEY);
