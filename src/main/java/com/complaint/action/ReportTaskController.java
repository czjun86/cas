...
import com.complaint.service.ReportTaskService;
import com.complaint.service.SysConfigService;
import com.complaint.service.TrackTaskService;
...
public class ReportTaskController {
	private static final Logger logger = LoggerFactory
			.getLogger(ReportTaskController.class);
	@Autowired
	...
	@Autowired
	private ReportTEService reportTEService;
	@Autowired
	...
	public ModelAndView rolelist(String serialno, String flowid,
		...
		if(value.equals("baidu")){
			mv= new ModelAndView("/reportTask/reportlist_baidu");
		}else{
			 mv = new ModelAndView("/reportTask/reportlist");
		}
		// 查询测试
		List<TestMasterlog> list = this.reportTaskService.getserialno(serialno, "",
				"", "",areaid,s_id);
		List<TestMasterlog> flowlist = this.reportTaskService.queryResult(serialno,
				"", netType, testType,areaid,s_id);
		List<ColourCode> colorlist = null;
		// HTTP,PING相关数据业务下
		if (flowlist != null && flowlist.size() > 0) {
			colorlist = this.reportTaskService.queryColor();
		}
		// 颜色
	...
	Map<String, Object> reportlist(String flowid, HttpServletRequest request,String areaid,String s_id) {
		...
		Map<String,Object> fmap = this.reportTaskService.getData(flowid,areaid,1,request);
		List<KpiFlshData> list = (List<KpiFlshData>) fmap.get("flist");
		List<ColourCode> colorlist = this.reportTaskService.queryColor();
		if (!flowid.equals("") && flowid.split(",").length == 1) {
			sernolist = this.reportTaskService.queryResult("", flowid, "", "",areaid,s_id);
			list2 = this.reportTaskService.getHPF(flowid,areaid);
			listmap.put("hplist", list2);
			listmap.put("colorlist", colorlist);
		}
		try {
			template = freemarkerConfig.getConfiguration().getTemplate(
					"/reportTask/http_ping.ftl");
	...
	public ModelAndView findFlowid(String serialno, String flowid,
		...
		ModelAndView mv = new ModelAndView("/reportTask/findFlowid");
		mv.addObject("serialno", serialno);
		mv.addObject("s_id", s_id);
		mv.addObject("areaid", areaid);
 ...
 * 报表共用的选择流水号树形菜单封装*/
	...
	List<ReportBean> initReport(HttpServletRequest request, String serialno,String s_id,
		...
		if (id == null || id == "") {
			List<TestMasterlog> list = this.reportTaskService.queryResult(serialno,
					"", "", "", areaid,s_id);
	...
	Map<String, Object> showPoint(String flowid, String type,
		...
		if (type != null && ("1".equals(type) || "3".equals(type))) {
			gsmlist = this.trackTaskService.queryPointGsm(flowid,areaid);
		}
		if (type != null && ("2".equals(type) || "3".equals(type))) {
			wcdmalist = this.trackTaskService.queryPointWcdma(flowid,areaid);
		}
		List<CenterZoom> center = this.trackTaskService.queryCenter(flowid,areaid);
		...
		try {
			...
			if (type!=null&&"1".equals(type)) {
				...
				template2 = freemarkerConfig.getConfiguration().getTemplate(
						"/reportTask/gsm_max.ftl");

				list2g = this.trackTaskService.queryGsm(id,areaid);
				listmap.put("list", list2g);
			} else if(type!=null){
				template = freemarkerConfig.getConfiguration().getTemplate(
						"/reportTask/wcdma.ftl");
				template2 = freemarkerConfig.getConfiguration().getTemplate(
						"/reportTask/wcdma_max.ftl");
				list3g = this.trackTaskService.queryWcdma(id,areaid);
	...
	// list2g=this.trackTaskService.queryGsm(id);
	// model.addAttribute("list", list2g);
	// }else{
	// rtnStr = "/reportTask/wcdma";
	// list3g=this.trackTaskService.queryWcdma(id);
	...
	public ModelAndView reportCompare(String serialno, String flowid,
		...
		ModelAndView mv =null;
		if(value.equals("baidu")){
			mv= new ModelAndView("/reportTask/reportCompare_baidu");
		}else{
			mv= new ModelAndView("/reportTask/reportCompare");
		}
	...
	Map<String, Object> reportCompareData(String flowid,String areaid,
		...
		Map<String, Object> map = new HashMap<String, Object>();
		// 柱状图数据
		Map<String,Object> fmap = this.reportTaskService.getData(flowid,areaid,2,request);
		List<KpiFlshData> list = (List<KpiFlshData>) fmap.get("flist");
		// 颜色
		List<ColourCode> colorlist = this.reportTaskService.queryColor();
		// 2G轨迹点
	...
	public ModelAndView indoorMap(String lat,String lng ,String flowid ,HttpServletRequest request){
		...
		ModelAndView mv =null;
		if(value.equals("baidu")){
			 mv = new ModelAndView("/reportTask/indoorMap_baidu");
		}else{
			 mv = new ModelAndView("/reportTask/indoorMap");
		}
	...
	public @ResponseBody Map<String ,Object> getCell(String flowid ,HttpServletRequest request){
		Map<String ,Object> map = this.reportTaskService.getCell(flowid );                                                                                                
		return map;
	}
	/**
	 * 显示小区详情
	...
	public String showAddress(HttpServletRequest request){
		return "/reportTask/updateAddress";
	}
	...
	public @ResponseBody Map<String, Integer> updateAddress(String address,String areaid,String flowid,HttpServletRequest request){
		...
		int it=this.reportTaskService.updateAdress(t);
	...
	public  @ResponseBody String reportExcel(String value,String graph ,String eval,String flowid ,String ping,HttpServletRequest request){
		...
		//模板路径
		String tempPath = request.getSession().getServletContext().getRealPath("/").replace("\\", "/")+Constant.CAS_TEMPLATE_PATH+"reportTaskExcel.xlsx";
		...
		//合并文件和文件名
		path = path+filename;
		reportTEService.excelService(valueMap , flowid, tempPath ,path ,valueMapgraph ,keys ,keyEvals ,evalMap , pingMap);
		return str;
	}
	/**
	 * 下载生成的Excel
	...
	public @ResponseBody  Map updateVerify(String id ,Integer val ,HttpServletRequest request ,HttpServletResponse response){
		...
		int flag = 0;
		try {
			flag = reportTaskService.updateVerify(id,val);
		} catch (Exception e) {
