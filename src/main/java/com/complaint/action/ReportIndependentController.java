...
import com.complaint.service.ReportIdExcelService;
import com.complaint.service.ReportIndependentService;
import com.complaint.service.ReportTEService;
import com.complaint.service.SysConfigService;
import com.complaint.service.TrackIndependentService;
...
@Controller
@RequestMapping("/reportIndependent")
public class ReportIndependentController {
	private static final Logger logger = LoggerFactory
			.getLogger(ReportIndependentController.class);
	@Autowired
	private ReportIndependentService reportIndependentService;

	@Autowired
	private TrackIndependentService trackIndependentService;
	...
	@Autowired
	private ReportIdExcelService reportIdExcelService;
	@Autowired
	...
	public ModelAndView rolelist(String serialno, String flowid,
		...
		if(value.equals("baidu")){
			mv= new ModelAndView("/reportIndependent/reportlist_baidu");
		}else{
			 mv = new ModelAndView("/reportIndependent/reportlist");
		}
		// 查询测试
		List<TestMasterlog> list = this.reportIndependentService.getserialno(serialno, "",
				"", "",areaid,s_id);
		List<TestMasterlog> flowlist = this.reportIndependentService.queryResult(serialno,
				"", netType, testType,areaid,s_id);
		List<ColourCode> colorlist = null;
		// HTTP,PING相关数据业务下
		if (flowlist != null && flowlist.size() > 0) {
			colorlist = this.reportIndependentService.queryColor();
		}
		// 颜色
	...
	Map<String, Object> reportlist(String flowid, HttpServletRequest request,String areaid,String s_id) {
		...
		Map<String,Object> fmap = this.reportIndependentService.getData(flowid,areaid,1,request);
		List<KpiFlshData> list = (List<KpiFlshData>) fmap.get("flist");
		List<ColourCode> colorlist = this.reportIndependentService.queryColor();
		if (!flowid.equals("") && flowid.split(",").length == 1) {
			sernolist = this.reportIndependentService.queryResult("", flowid, "", "",areaid,s_id);
			list2 = this.reportIndependentService.getHPF(flowid,areaid);
			listmap.put("hplist", list2);
			listmap.put("colorlist", colorlist);
		}
		try {
			template = freemarkerConfig.getConfiguration().getTemplate(
					"/reportIndependent/http_ping.ftl");
	...
	public ModelAndView findFlowid(String serialno, String flowid,
		...
		ModelAndView mv = new ModelAndView("/reportIndependent/findFlowid");
		mv.addObject("serialno", serialno);
		mv.addObject("s_id", s_id);
		mv.addObject("areaid", areaid);
 ...
 * 报表共用的选择流水号树形菜单封装*/
	...
	List<ReportBean> initReport(HttpServletRequest request, String serialno,String s_id,
		...
		if (id == null || id == "") {
			List<TestMasterlog> list = this.reportIndependentService.queryResult(serialno,
					"", "", "", areaid,s_id);
	...
	Map<String, Object> showPoint(String flowid, String type,
		...
		if (type != null && ("1".equals(type) || "3".equals(type))) {
			gsmlist = this.trackIndependentService.queryPointGsm(flowid,areaid);
		}
		if (type != null && ("2".equals(type) || "3".equals(type))) {
			wcdmalist = this.trackIndependentService.queryPointWcdma(flowid,areaid);
		}
		List<CenterZoom> center = this.trackIndependentService.queryCenter(flowid,areaid);
	...
	Map<String, Object> wcdmaOrGsm(String type, String id,String areaid,
		...
		try {
			...
			if (type!=null&&"1".equals(type)) {
				template = freemarkerConfig.getConfiguration().getTemplate(
						"/reportIndependent/gsm.ftl");
				template2 = freemarkerConfig.getConfiguration().getTemplate(
						"/reportIndependent/gsm_max.ftl");

				list2g = this.trackIndependentService.queryGsm(id,areaid);
				listmap.put("list", list2g);
			} else if(type!=null){
				template = freemarkerConfig.getConfiguration().getTemplate(
						"/reportIndependent/wcdma.ftl");
				template2 = freemarkerConfig.getConfiguration().getTemplate(
						"/reportIndependent/wcdma_max.ftl");
				list3g = this.trackIndependentService.queryWcdma(id,areaid);
	...
	// */
	// @RequestMapping(value = "/wcdmaOrGsm",method = RequestMethod.POST)
	// public String wcdmaOrGsm(Model model,String type,String
	...
	// try {
	// if(type.equals("1")){
	// rtnStr = "/reportTask/gsm";
	// list2g=this.trackIndependentService.queryGsm(id);
	// model.addAttribute("list", list2g);
	// }else{
	// rtnStr = "/reportTask/wcdma";
	// list3g=this.trackIndependentService.queryWcdma(id);
	// model.addAttribute("list", list3g);
	// }
	// } catch (Exception e) {
	// e.printStackTrace();
	// logger.error("",e);
	...
	public ModelAndView reportCompare(String serialno, String flowid,
		...
		ModelAndView mv =null;
		if(value.equals("baidu")){
			mv= new ModelAndView("/reportIndependent/reportCompare_baidu");
		}else{
			mv= new ModelAndView("/reportIndependent/reportCompare");
		}
	...
	Map<String, Object> reportCompareData(String flowid,String areaid,
		...
		Map<String, Object> map = new HashMap<String, Object>();
		// 柱状图数据
		Map<String,Object> fmap = this.reportIndependentService.getData(flowid,areaid,2,request);
		List<KpiFlshData> list = (List<KpiFlshData>) fmap.get("flist");
		// 颜色
		List<ColourCode> colorlist = this.reportIndependentService.queryColor();
		// 2G轨迹点
	...
	public ModelAndView indoorMap(String lat,String lng ,String flowid ,HttpServletRequest request){
		...
		ModelAndView mv =null;
		if(value.equals("baidu")){
			 mv = new ModelAndView("/reportIndependent/indoorMap_baidu");
		}else{
			 mv = new ModelAndView("/reportIndependent/indoorMap");
		}
	...
	public @ResponseBody Map<String ,Object> getCell(String flowid ,HttpServletRequest request){
		Map<String ,Object> map = this.reportIndependentService.getCell(flowid );                                                                                                
		return map;
	}
	/**
	 * 显示小区详情
	...
	public String showAddress(HttpServletRequest request){
		return "/reportIndependent/updateAddress";
	}
	...
	public @ResponseBody Map<String, Integer> updateAddress(String address,String areaid,String flowid,HttpServletRequest request){
		...
		int it=this.reportIndependentService.updateAdress(t);
	...
	public  @ResponseBody String reportExcel(String value,String graph ,String eval,String flowid ,String ping,HttpServletRequest request){
		...
		//模板路径
		String tempPath = request.getSession().getServletContext().getRealPath("/").replace("\\", "/")+Constant.CAS_TEMPLATE_PATH+"reportIndependentExcel.xlsx";
		...
		//合并文件和文件名
		path = path+filename;
		reportIdExcelService.excelService(valueMap , flowid, tempPath ,path ,valueMapgraph ,keys ,keyEvals ,evalMap , pingMap);
		return str;
	}
	/**
	 * 下载生成的Excel
	...
	public @ResponseBody  Map updateVerify(String id ,Integer val ,HttpServletRequest request ,HttpServletResponse response){
		...
		int flag = 0;
		try {
			flag = reportIndependentService.updateVerify(id,val);
		} catch (Exception e) {
